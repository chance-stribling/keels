...
class TMDBController extends Controller
{
    ...
    public function fetchAllMovies(Request $request)
    {
        ...
        $allMovies = collect();
        $maxPages = 10; // Limit to first 10 pages (200 results max)
        ...
        try {
            ...
            // If there is a search term, only return movies that start with the search term
            if ($search) {
                $allMovies = $allMovies->filter(function ($movie) use ($search) {
                    return stripos($movie['title'], $search) === 0; // Exact match at the beginning
                });
            }
            ...
            // Pagination: Slice the final collection after filtering and sorting
